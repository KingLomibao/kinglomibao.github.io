...
OpsPilot uses PostgreSQL. The schema's source of truth is the
...
sudo -u postgres psql -c "CREATE USER opspilot WITH PASSWORD 'opspilot' CREATEDB;"
sudo -u postgres psql -c "CREATE DATABASE opspilot OWNER opspilot;"
sudo -u postgres psql -c "CREATE DATABASE opspilot_test OWNER opspilot;"
...
`opspilot_test` is used by the pytest suite (see `tests/backend/conftest.py`) and is never touched by the seed script.
